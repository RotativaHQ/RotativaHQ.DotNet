...
namespace RotativaHQ.Core
{
    public class AssetPathCollection
    {
        ...
        public void AddSerializedAsset(IElement element, string uriAttribute)
        {
            ...
            var localPath = Zipper.ReturnLocalPath(src);
        ...
        public void AddSerializedAssets(
            ...
            )
        {
            ...
            {
                this.AddSerializedAsset(element, uriAttribute);
            }
        }
    ...
    public class RhqPackageBuilder: IDisposable
    {
        ...
        private AssetPathCollection ExtractAssets(string source, string suffix, string pagePath, AssetPathCollection assets)
        {
            if (suffix.ToLower().StartsWith("htm"))
            {
                ...
                {
                    var content = Zipper.GetStringAsset(asset.OriginalPath, mapPathResolver, webRoot, pagePath);
                    var binaryContent = Encoding.UTF8.GetBytes(content);
                    asset.Content = binaryContent;
                    ExtractAssets(content, "css", asset.OriginalPath, assets);
                }

                foreach (var asset in assets.Assets.Where(a => a.AssetType != "css"))
                {
                    var binaryContent = Zipper.GetBinaryAsset(asset.OriginalPath, mapPathResolver, webRoot, pagePath);
                    asset.Content = binaryContent;
                }
            ...
            else if (suffix.ToLower() == "css")
            {
                ...
                {
                    var csslocalPath = Zipper.ReturnLocalPath(url);
